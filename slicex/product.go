...
// 求：数组A、B、C的笛卡尔积
//
// 解：分别从每一个数组中取出一个元素，然后组成一个新的数组，总共会产生 4*4*2=32 个新的数组组合。
//
// 可能产生新的数组组合如下：
//
...
func CartesianProduct[T any](slice [][]T) [][]T {
	...
	var product = make([][]T, 0, total)

	var n = len(slice)
	for index := 0; index < total; index++ {
		var elements = make([]T, 0, n)
		var temp = index
		for m := 0; m < n; m++ {
			var pLen = len(slice[m])
			elements = append(elements, slice[m][temp%pLen])
			temp /= pLen
		}
		product = append(product, elements)
	}
	return product
}
